...
class Solution:
    def isLongPressedName(self, name: str, typed: str) -> bool:
        left, len_name = 0, len(name)
        for i, ch in enumerate(typed):
            if left < len_name and name[left] == ch:
                left += 1
            else:
                if i == 0 or typed[i] != typed[i - 1]:
                    return False
        return left == len_name


if __name__ == '__main__':
    assert not Solution().isLongPressedName("pyplrz", "ppyypllr")
    assert not Solution().isLongPressedName("alex", "alexxr")
    assert Solution().isLongPressedName("alex", "aaleex")
    assert not Solution().isLongPressedName("alex", "aaleelx")
