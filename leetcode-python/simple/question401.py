...
class Solution:
    ...
    def readBinaryWatchv2(self, num: int) -> List[str]:
        def calc_hours(bit: List[int]) -> int:
            hour_num = 0
            for i in bit:
                if i >= 6:
                    hour_num += bit_val[i]
        ...
        def search(bit: List[int], start_num: int, hour_num: int, min_num: int):
            if len(bit) == num:
                valid_time = str(hour_num) + ":"
                if min_num < 10:
                    valid_time += "0"
                result.append(valid_time + str(min_num))
                return
            for i in range(start_num, 10):
                bit.append(i)
                hour_num, min_num = calc_hours(bit), calc_min(bit)
                if hour_num > 11 or min_num > 59:
                    bit.remove(i)
                    continue
                search(bit, i + 1, hour_num, min_num)
                bit.remove(i)

        bit_val = [1, 2, 4, 8, 16, 32, 1, 2, 4, 8]
        result = []
        search([], 0, 0, 0)
        return result
...
if __name__ == '__main__':
    for i in range(0, 11):
        result = Solution().readBinaryWatchv2(i)
        print('if num == ' + str(i) + ":")
        print('    return ', result, '')
