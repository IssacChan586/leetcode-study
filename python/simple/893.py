...
class Solution:
    def numSpecialEquivGroups(self, A: List[str]) -> int:
        all_ch, all_group = 'qwertyuiopasdfghjklzxcvbnm', set()
        for word in A:
            count = [0 for _ in range(52)]
            for i in range(len(word)):
                count[ord(word[i]) - ord('a') + 26 * (i % 2)] += 1
            all_group.add(tuple(count))
        return len(all_group)


if __name__ == '__main__':
    assert Solution().numSpecialEquivGroups(["abcd", "cdab", "cbad", "xyzz", "zzxy", "zzyx"]) == 3
    assert Solution().numSpecialEquivGroups(["abc", "acb", "bac", "bca", "cab", "cba"]) == 3
