...
class Solution:
    def simplifyPath(self, path: str) -> str:
        file_path = []
        for each_part in path.split("/"):
            if each_part not in ['', '.', '..']:
                file_path.append(each_part)
            elif each_part == '..' and file_path:
                file_path.pop()
        return '/' + '/'.join(file_path)

    def simplifyPathV2(self, path: str) -> str:
...
if __name__ == '__main__':
    ...
    validator.validate(Solution().simplifyPath("/a/../../b/../c//.//"), "/c")
    validator.validate(Solution().simplifyPath("/a//b////c/d//././/.."), "/a/b/c")
